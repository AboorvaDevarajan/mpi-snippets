...
int main(int argc, char **argv) {
  ...
  // CASE: 4 MPI_SUM origin : derived dt (non-contig) source : derived dt
        ...
        for (i = 0; i < (2 * count); i++) {
          if (i % 2 == 0) {
            if (res_ptr[i] != peer || origin_ptr[i] != (rank * 1000) ||
                win_ptr[i] != rank) {
              printf("CASE 41: rank : %d result : %d expected : %d win_ptr : %d "
            ...
            if (res_ptr[i] != (rank * 1000) || origin_ptr[i] != (rank * 100) ||
                win_ptr[i] != 0) {
              printf("CASE 42: rank : %d result : %d expected : %d win_ptr : %d "
                     "expected: %d origin_ptr %d expected: %d\n",
        ...
        for (i = 0; i < (2 * count); i++) {
          if (i % 2 == 0) {
            if (res_ptr[i] != (rank * 100) || origin_ptr[i] != (rank * 1000) ||
                win_ptr[i] != (((rank - 1)*1000) + rank)) {
              printf("CASE 43: rank : %d result : %d expected : %d win_ptr : %d "
                     "expected: %d origin_ptr %d expected: %d\n",
                     rank, res_ptr[i], (rank * 100), win_ptr[i], (((rank - 1)*1000) + rank),
                     origin_ptr[i], rank * 1000);
              exit(1);
            }
          } else {
            if (res_ptr[i] != (rank * 1000) || origin_ptr[i] != (rank * 100) ||
                win_ptr[i] != 0) {
              printf("CASE 44: rank : %d result : %d expected : %d win_ptr : %d "
                     "expected: %d origin_ptr %d expected: %d\n",
